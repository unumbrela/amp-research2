"""Phase 2 Training: VAE with ESM-2 embeddings, contrastive loss, and property prediction.

v4: Autoregressive GRU decoder with teacher forcing and scheduled sampling.
"""
...
from training.utils import (
    ...
    TrainingLogger, EarlyStopping, ModelEMA,
)
...
def _forward_batch(model, batch, device, teacher_forcing_ratio=1.0):
    """Run forward pass with pre-computed ESM embeddings."""
    ...
    one_hot_dropout_prob = float(getattr(_forward_batch, "one_hot_dropout_prob", 0.0))
    esm_noise_std = float(getattr(_forward_batch, "esm_noise_std", 0.0))

    if model.training and one_hot_dropout_prob > 0:
        keep_mask = (torch.rand_like(one_hot[..., :1]) > one_hot_dropout_prob).float()
        one_hot = one_hot * keep_mask

    has_esm = bool((esm_emb.abs().sum() > 0).item())
    if has_esm:
        if model.training and esm_noise_std > 0:
            esm_emb = esm_emb + torch.randn_like(esm_emb) * esm_noise_std
    else:
        sequences = batch["sequence"]
        esm_emb = model.esm(sequences, max_len=one_hot.size(1)).to(device)
        if model.training and esm_noise_std > 0:
            esm_emb = esm_emb + torch.randn_like(esm_emb) * esm_noise_std

    mu, logvar = model.encoder(one_hot, esm_emb, padding_mask)
    z = model.reparameterize(mu, logvar)
    logits, length_logits = model.decode(
        z, properties,
        target_indices=target_indices if model.training else None,
        teacher_forcing_ratio=teacher_forcing_ratio if model.training else 0.0,
    )
...
def main():
    ...
    print(f"=== ESM-DiffVAE v4 VAE Training ===")
    print(f"Device: {device}")
    vae_cfg = config["vae"]
    print(f"Architecture: BiGRU encoder (h={vae_cfg['hidden_dim']}) + "
          f"GRU decoder (h={vae_cfg.get('decoder_hidden_dim', vae_cfg['hidden_dim'])})")
    ...
    val_loader = create_dataloader(
        ...
        data_dir / "embeddings" / "val_esm.pt",
        max_len=config["vae"]["max_seq_len"],
        batch_size=config["train_vae"]["batch_size"],
        shuffle=False,
    )
    ...
    _forward_batch.one_hot_dropout_prob = config["train_vae"].get("one_hot_dropout_prob", 0.0)
